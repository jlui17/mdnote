...
- **Frontend:** one page, vanilla TypeScript bundled by Bun. No React, no build pipeline beyond `bun build`. Selection via `window.getSelection()`, resolved to source anchors by walking up to the nearest `data-source-line` element and computing offsets within it.
