...
The page opens in dark mode, or whatever `theme` you set in [Settings](#settings). The theme picker in the sidebar header switches between mdnote's own light/dark/system modes and a set of palettes mirroring [daisyUI's themes](https://daisyui.com/docs/themes/) of the same name (dracula, synthwave, nord, cupcake, …). The ▬ button next to it (or `r`) turns on a reading line: a band one text line tall that follows your pointer through the document and stays on the line you left it on while you scroll. Both choices are written back to your settings file, so they stick across files, tabs, and reloads.
...
- **`readingLine`** — `true` turns on the reading line (default `false`): a band one text line tall behind whichever line the pointer is on, spanning the document's width. It moves only when the pointer does, so scrolling leaves it on the line it marked.
- **`keybindings`** — action → shortcut, merged over the defaults; `null` unbinds a default. A spec is `mod`/`shift`/`alt` modifiers plus a key, joined by `+` (`mod` is ⌘ on Mac, Ctrl elsewhere). Actions: `annotate-block` (default `c`), `annotate-document` (default `shift+c`), `edit-annotation` (default `e`), `delete-annotation` (default `shift+d`), `toggle-reading-line` (default `r`), `submit-review` (default `mod+enter`), `show-help` (default `shift+?`), and `copy-markdown` and `toggle-theme` (steps to the next theme in the list above; unbound by default).
