...
- **`readingLine`** — `true` turns on the reading line (default `false`): a band one text line tall over whichever line the pointer is on, spanning the document's width. It moves only when the pointer does or the arrows step it (each step recenters the page on it), so plain scrolling leaves it on the line it marked.
- **`generalNoteSize`** — `{ "width", "height" }` in pixels, the size of the general note dialog (absent by default: the dialog opens at its built-in size). Resizing the dialog writes it, rounded to whole pixels; a size too big for the window is shown clamped to fit, without changing the saved value.
...
An invalid entry warns in the server log (`~/.local/state/mdnote/server.log`, truncated each time the server starts) and falls back to the default for that key. Edits apply on page reload; no server restart needed. The theme picker, the reading-line toggle, and a resize of the general note dialog write `theme`, `readingLine`, and `generalNoteSize` back into this file (creating it if needed) and leave every other key as you wrote it; a file that isn't valid JSON is left alone, and the page says so, until you fix it.
